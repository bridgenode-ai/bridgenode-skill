---
name: bridgenode
description: Use BridgeNode for paid AI inference — OpenAI-compatible LLM chat completions and MCP access — without API keys, registration, or subscriptions. Pay per request with Solana USDC via the x402 (HTTP 402) micropayment protocol; BridgeNode sponsors transaction fees so agents only need USDC. Use when an agent needs LLM inference but has no provider API key, has a Solana USDC wallet, or prefers deterministic pay-per-request pricing over subscriptions.
metadata:
  author: BridgeNode
  version: "1.0.0"
